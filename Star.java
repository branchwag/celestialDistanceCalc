...
public class Star extends CelestialObject {
	...
	private double absoluteMagnitude; // true brightness, M = m - 5log10(d) + 5 where m is how bright it looks from
						// earth and d is distance. Ex. Sun is +4.83 absolute mag with -26.74
						// apparent mag
	private String spectralType; // classification based on Morgan-Keenan (MK) system - Letter, Number,
					// Luminosity
	...
	// I = Supergiant, II = Bright Giant, III = Giant, IV = Subgiant, V =
	// Main-sequence star
	// Ex. Sun is G2V - dwarf star in G2 category

	// CONSTRUCTOR
	public Star(String name, double rightAscension, double declination, double apparentMagnitude, double parallax,
			double absoluteMagnitude, String spectralType) {
	...
	// function to calc distance in light years using spectroscopic parallax method
	// - i.e. compare absolute mag to apparent mag
	...
	public String getInfo() {
		...
		if (parallax > 0) {
			...
			info.append("Distance (parallax method): ")
					.append(String.format("%.2f", calcParallaxDistance())).append(" light years\n");
		}

		info.append("Distance (spectroscopic method): ").append(String.format("%.2f", calcSpecDistance()))
				.append(" light years");
	...
	private static Color getColorForSpectralType(String spectralType) {
		...
		switch (type) {
			case 'O':
				return new Color(155, 176, 255); // Blue
			case 'B':
				return new Color(170, 191, 255); // Blue-white
			case 'A':
				return new Color(202, 215, 255); // White
			case 'F':
				return new Color(248, 247, 255); // Yellow-white
			case 'G':
				return new Color(255, 244, 234); // Yellow
			case 'K':
				return new Color(255, 210, 161); // Orange
			case 'M':
				return new Color(255, 160, 142); // Red
			default:
				return Color.WHITE;
		}
	}
